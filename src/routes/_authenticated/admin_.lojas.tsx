  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Lojas e campanhas | Vitrini" },
      {
        ...
        content: "Gerencie lojas e campanhas para vendedores do Vitrini.",
      },
...
function AdminStores() {
  ...
  const visibleStores = useMemo(() => {
    ...
    return stores.filter((store) => {
      const matchesSearch =
        !term ||
        [store.name, store.slug, store.sellerName].some((value) =>
          value.toLowerCase().includes(term),
        );
      return (
        matchesSearch &&
        (plan === "all" || store.plan === plan) &&
        (status === "all" || (store.isActive ? "active" : "inactive") === status)
      );
    });
  }, [plan, search, status, stores]);
  const chosenStores = visibleStores.filter((store) => selected.includes(store.id));
  const currentTemplate =
    ADMIN_TEMPLATES.find((item) => item.value === template) ?? ADMIN_TEMPLATES[0];
  ...
  function openWhatsApp(store: AdminCampaignStore) {
    const text = fillTemplate(message, {
      lojista: store.sellerName || "vendedor",
      loja: store.name,
      ...fields,
    });
  ...
  if (adminLoading || isLoading)
    return (
      <AppShell title="Lojas e campanhas">
      ...
      <div className="space-y-5">
        <section className="grid gap-3 sm:grid-cols-3">
          ...
          <div className="surface flex items-center gap-3 p-4">
            ...
            <div>
              ...
              <p className="text-xl font-bold">{chosenStores.length}</p>
            </div>
          </div>
          <div className="surface flex items-center gap-3 p-4">
            <Send className="size-5 text-primary" />
            <div>
              <p className="text-xs text-muted-foreground">Abertas nesta campanha</p>
        ...
        <section className="surface space-y-4 p-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <h2 className="font-semibold">Campanha para vendedores</h2>
              <p className="text-sm text-muted-foreground">
                Selecione as lojas e abra uma conversa pronta.
              </p>
            </div>
            <Badge variant="secondary">{chosenStores.length} selecionada(s)</Badge>
          </div>
          ...
          <p className="text-xs text-muted-foreground">
            A mensagem será personalizada com o nome do vendedor e da loja.
          </p>
        </section>
        ...
        <section className="surface overflow-hidden">
          <div className="flex flex-col gap-3 border-b border-border p-4 lg:flex-row lg:items-center">
            <div className="relative flex-1">
              ...
              <Input
                className="pl-9"
                placeholder="Buscar loja ou vendedor"
                value={search}
            ...
            <Button variant="outline" onClick={selectVisible}>
              {visibleStores.length > 0 &&
              visibleStores.every((store) => selected.includes(store.id)) ? (
          ...
          <div className="divide-y divide-border">
            {visibleStores.map((store) => {
              ...
              const wasSent = sent.includes(store.id);
              return (
                <div key={store.id} className="flex flex-wrap items-center gap-3 p-4">
                  <button
                    ...
                    className={`flex size-5 items-center justify-center rounded border ${isSelected ? "border-primary bg-primary text-primary-foreground" : "border-border"}`}
                  >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="truncate text-xs text-muted-foreground">
                      {store.sellerName} · /s/{store.slug} · {formatDate(store.createdAt)}
                    </p>
                  </div>
