      ...
      <div className="space-y-5">
        ...
        <section className="surface overflow-hidden">
          ...
          <div className="divide-y divide-border">
            {visibleStores.map((store) => { const isSelected = selected.includes(store.id); const wasSent = sent.includes(store.id); return <div key={store.id} className="flex flex-wrap items-center gap-3 p-4"><button type="button" aria-label={`Selecionar ${store.name}`} onClick={() => setSelected((current) => isSelected ? current.filter((id) => id !== store.id) : [...current, store.id])} className={`flex size-5 items-center justify-center rounded border ${isSelected ? "border-primary bg-primary text-primary-foreground" : "border-border"}`}>{isSelected ? <Check className="size-3" /> : null}</button><Store className="size-5 text-muted-foreground" /><div className="min-w-0 flex-1"><p className="truncate font-medium">{store.name}</p><p className="truncate text-xs text-muted-foreground">{store.sellerName} · /loja/{store.slug} · {formatDate(store.createdAt)}</p></div><Badge variant={store.plan === "pro" ? "default" : "secondary"}>{store.plan}</Badge><Badge variant={store.isActive ? "secondary" : "destructive"}>{store.isActive ? "ativa" : "inativa"}</Badge><Button size="sm" variant={wasSent ? "outline" : "default"} onClick={() => openWhatsApp(store)}><MessageCircle className="mr-1.5 size-4" />{wasSent ? "Abrir novamente" : "Abrir WhatsApp"}</Button></div>; })}
